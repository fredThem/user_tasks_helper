...
export const v5DiagnoseDeps = () => {
  ...
  console.log(
    '🤖 ~ file: v5-diagnose-deps.js:16 ~ v5DiagnoseDeps ~ sourcePath',
    sourcePath,
  );
  ...
  const v5PackageJson = JSON.parse(
    fs.readFileSync(
      path.resolve(sourcePath, 'node_modules/@cbcradcan/vcinq/package.json'),
      'utf8',
    ),
  );
  ...
  async function initUpdate() {
    ...
    if (response === 'update') {
      ...
      if (responseConfirm) {
        ...
        const command = `cd ${sourcePath} &&
      npm install ${dependenciesToUpdateString} --legacy-peer-deps`;
